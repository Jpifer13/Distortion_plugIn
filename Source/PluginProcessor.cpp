...
#endif
{
	...
	state->createAndAddParameter("volume", "Volume", "Volume", NormalisableRange<float>(0.0, 3.0, 0.0001), 1.0, nullptr, nullptr);
...
void DistortionVstAudioProcessor::processBlock (AudioBuffer<float>& buffer, MidiBuffer& midiMessages)
{
    ...
    for (int channel = 0; channel < totalNumInputChannels; ++channel)
    {
		...
		//this is the actual loop to distort the signal
		for (int sample = 0; sample < buffer.getNumSamples(); sample++) {

			float cleanSig = *channelData;//this saves the original clean signal

			*channelData *= drive * range;//this decides the range we can increase the drive to. how loud it can get

			*channelData = (((((2.f / float_Pi) * atan(*channelData)) * blend) + (cleanSig * (1.f - blend))) / 2.f) * volume;//this creates the distorted signal and blends it into the clean signal by taking the average of the two then multiplying  it by the data
